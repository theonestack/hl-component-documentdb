...
describe 'compiled component documentdb' do
  ...
  context 'cftest' do
    it 'compiles test' do
      expect(system("cfhighlander cftest #{@validate} --tests tests/encryption.test.yaml")).to be_truthy
    end      
  end
  
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/encryption/documentdb.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "SSMSecureParameterCR" do
      ...
      it "to have property Runtime" do
          expect(resource["Properties"]["Runtime"]).to eq("python3.u8")
      end
