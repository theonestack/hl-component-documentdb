...
describe 'compiled component documentdb' do
  ...
  let(:template) { YAML.load_file("#{File.dirname(__FILE__)}/../out/tests/tags/documentdb.compiled.yaml") }
  ...
  context "Resource" do
    ...
    context "DocDBSecurityGroup" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-documentdb"}}, {"Key"=>{"Fn::Sub"=>"CostCenter"}, "Value"=>{"Fn::Sub"=>"${CostCenter}"}}])
      end
    ...
    context "DocDBCluster" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-documentdb-cluster"}}, {"Key"=>{"Fn::Sub"=>"CostCenter"}, "Value"=>{"Fn::Sub"=>"${CostCenter}"}}])
      end
    ...
    context "DocDBInstanceA" do
      ...
      it "to have property Tags" do
          expect(resource["Properties"]["Tags"]).to eq([{"Key"=>"Name", "Value"=>{"Fn::Sub"=>"${EnvironmentName}-documentdb-instance-A"}}, {"Key"=>{"Fn::Sub"=>"CostCenter"}, "Value"=>{"Fn::Sub"=>"${CostCenter}"}}])
      end
